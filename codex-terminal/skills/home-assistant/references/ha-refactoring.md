...
> Pentru structuri storage/dashboard dependente de versiunea HA, verifică [ha-version-notes.md](ha-version-notes.md) înainte de schimbări critice.
...
4. **Rulează `ha-context --force`** ca `rename_memory.json` să păstreze noua stare a registrului.
5. **Dacă rămân referințe stale pe care nu le poți actualiza** — redenumește entitatea înapoi la ID-ul original pentru a restabili funcționalitatea, apoi raportează locațiile blocante utilizatorului.
...
Rulează `ha-context --force`, apoi verifică intrările relevante din `/data/ha-context/rename_memory.json`: entity_id-urile noi trebuie să apară cu friendly names canonical, iar cele dezactivate cu `disabled_by` setat.

---

**TL;DR:** Citește `rename_memory.json`, sari peste ce e deja canonical, apoi caută toți consumatorii ÎNAINTE de orice schimbare. Verifică zero referințe stale DUPĂ și rulează `ha-context --force`. Acceptă prompt-ul din UI la redenumire (actualizează 90% automat). Redenumirile din registry NU actualizează: Config-Entry-Groups (`options.entities`), Config-Entry-Data (`data`/`options` ale integrațiilor), storage dashboards (`.storage/lovelace.*`) — acestea necesită pași manuali separați descriși în secțiunile de mai sus.
