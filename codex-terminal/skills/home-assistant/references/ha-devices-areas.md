...
2. Adaugi noul label în lista canonică din această secțiune, în categoria potrivită (`name`, `slug`, `category`, `icon`).
3. Rulezi `ha-context --force` după aplicarea label-ului în HA, ca `rename_memory.json` să reflecte starea nouă.
...
- **Folosește entity_id-uri descriptive** — `light.living_ceiling` și `light.living_tv_led` îți spun imediat care e care.
...
- `core.area_registry` și `core.label_registry` — nume și metadate pentru arii/labels.
- `/api/states` — `friendly_name`, `device_class`, unități și stare curentă.
...
5. Verifică apoi că intrările relevante din `rename_memory.json` conțin numele nou, aria, labels și `disabled_by` corecte.
...
8. **NU schimba slug-ul de cameră** după ce e folosit în automatizări fără search&replace complet.
9. **Dezactivează entitățile nefolosite** pentru fiecare device atins (vezi `ha-entities.md`).
10. **Rulează `ha-context --force` după fiecare batch** — nu lăsa refresh-ul pe mai târziu, fiindcă următoarea sesiune se bazează pe memoria generată.

---

**TL;DR:** Areas sunt fixe — slug în română (`living`, `dormitor1`). Labels din lista canonică — NU creezi label nou dacă există sinonim. Un label primar per device; mai multe doar dacă funcțiile sunt cu adevărat distincte. Device name: `[Cameră] Producător Model [#N]` — brand/model în scriere oficială, fără funcție. Citește `rename_memory.json` înainte de redenumiri, sari peste ce e deja canonical, apoi rulează `ha-context --force`.
