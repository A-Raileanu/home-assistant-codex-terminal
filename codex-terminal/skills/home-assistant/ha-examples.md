...
- **Entity name (RO):** funcția în română din vocabularul `ha-entities.md` (`Putere`, `Mișcare`, `Temperatură`). Dacă nu există în vocabular, folosești un termen descriptiv nou în română și îl documentezi în `change_log`. Abrevieri tehnice (CO2, TVOC, RSSI) rămân neschimbate.
...
- **La final:** actualizează `inventory.yaml` (devices + labels + entități dezactivate + change_log cu numărul de referințe actualizate).
