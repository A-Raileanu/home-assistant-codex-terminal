...
show_help() {
    echo "persist-install - install packages on every add-on start"
    echo ""
    echo "Usage:"
...
install_apk() {
    init_config
    [ "$#" -gt 0 ] || { echo "No APK packages specified" >&2; exit 1; }
    apk add --no-cache "$@"
    for pkg in "$@"; do
...
install_pip() {
    init_config
    [ "$#" -gt 0 ] || { echo "No pip packages specified" >&2; exit 1; }
...
remove_package() {
    ...
    [ -n "$type" ] && [ -n "$pkg" ] || { echo "Usage: persist-install remove <apk|pip> <package>" >&2; exit 1; }
